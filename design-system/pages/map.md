...
- **Language:** right tool stack, under zoom/layers. Desktop 2×2 한/EN/日/中; pressed option uses `accent-soft`. Compact: toggle under layers, menu **below** the toggle. Not on the location FAB.
...
- Tap a list row to open `#place-detail` and zoom the map to that pin (level 3). Back is a 32px left chevron (`aria-label` only). Back/Escape restores the previous center and level. Pin tap opens the same panel without changing the camera. Desktop: photo left, aside with address, reserved yesterday/today/tomorrow forecast (hidden only when GPS is within 3km), badges, hours. Overview full width under `.detail-top`. Leftover snapshot fields in `.detail-extra` at the bottom. Compact: photo on top, then the aside. Same 3-snap sheet heights. Brand search does not use this panel.
