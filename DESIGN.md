...
지도가 캔버스다. UI는 그 위에 뜬 밝은 크롬이다. 검색바, 브랜드 칩, 현재 동 표시, 우측 지도 도구, 하단 장소 시트가 한 가족처럼 같은 흰 표면·잉크 글자·짧은 그림자를 쓴다.
...
**The One Accent Rule.** 위치와 키보드 포커스만 파랑이다. 검색 버튼과 활성 칩은 잉크다. 생성기가 제안한 이벤트 오렌지·크림 배경은 쓰지 않는다.
...
- **칩:** 브랜드 아이콘을 오른쪽에 두고, 활성일 때 잉크 채움.
- **도구 버튼:** 세로로 붙고 활성 행만 잉크. 컴팩트에서는 레이어 토글 아래에 타입·오버레이 패널.
- **시트:** 핸들 바, 제목+건수, 썸네일 번호가 있는 장소 행. 활성 행은 `surface-muted`.
- **핀/클러스터:** 흰 콘 핀과 검정 숫자 원. 선택 시 검정 말풍선 상호.
