...
**The One Accent Rule.** 위치와 키보드 포커스가 파랑이다. 검색 버튼과 활성 브랜드/명소 칩은 잉크다. **내 근처**만 눌린 옵션(`aria-pressed`)으로 `accent-soft`를 쓴다. 생성기가 제안한 이벤트 오렌지·크림 배경은 쓰지 않는다.
...
좌상단 검색 열(데스크톱 최대 560px), 그 아래 가로 스크롤 칩, 그 아래 **왼쪽 정렬**된 캡슐형 현재 주소(`width: fit-content`, 최대 280px). 우측 상단은 줌·지도 타입·오버레이 스택. 현위치는 검색 행이 아니라 **우측 하단 플로팅 버튼**이다. 하단은 전체 너비 바텀시트. 맵은 `100% / 100dvh`.
...
- **도구 버튼:** 세로로 붙고 활성 행만 잉크. 컴팩트에서는 레이어 토글 아래에 타입·오버레이 패널.
- **시트:** 핸들 바, 제목+건수, 썸네일 번호가 있는 장소 행. 명소·관광 썸네일은 대표 사진(실패 시 Carto). 명소 행은 유네스코/국가유산/유적 뱃지와 한 줄 소개. 활성 행은 `surface-muted`. 상세 시트는 같은 흰 패널에 뒤로 가기, 제목·메타 아래 장소 날씨(현위치와 3km 밖이거나 GPS가 없을 때), 대표 사진, 개요, 이용 정보. 브랜드는 상세가 없다.
